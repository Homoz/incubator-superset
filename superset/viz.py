...
class ChinaMapViz(BaseViz):
    """A visualization with China map background"""
    viz_type = "china_map"
    verbose_name = _("China map")
    is_timeseries = False

    def query_obj(self):
        query = super(ChinaMapViz, self).query_obj()
        fd = self.form_data
        query.get('groupby').extend([fd.get('all_columns_x'), fd.get('all_columns_y')])
        return query

    def get_data(self, df):
        fd = self.form_data
        lon_idx = fd.get('all_columns_x')
        lat_idx = fd.get('all_columns_y')
        city_name_idx = fd.get('groupby')[0]
        value_idx = fd.get('metrics')[0]
        geo_coord_map = {}
        cities_with_values = []
        #  TODO: df.itertuples() is faster?
        for idx, row in df.iterrows():
            geo_coord_map[row[city_name_idx]] = [row[lon_idx], row[lat_idx]]
            cities_with_values.append({'name':row[city_name_idx], 'value': row[value_idx]})

        return {
                'geoCoordMap': geo_coord_map,
                'citiesWithValues' : cities_with_values
        }
